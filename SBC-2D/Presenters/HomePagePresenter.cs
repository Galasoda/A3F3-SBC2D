...
namespace SBC_2D.Presenters
{
    public class HomePagePresenter
    {
        ...
        public HomePagePresenter(IHomePageView view, Machine machine)
        {
            _view = view;
            _machine = machine;
        }
        ...
        public void Dispose()
        {
            ...
            _machine.OnStatusChanged -= MachineContext_StatusChanged;
            _machine.SystemIo.SystemDisUpdated -= _view.MachineDiagramView.SystemDisUpdated;
            _machine.SystemIo.SystemDosUpdated -= _view.MachineDiagramView.SystemDosUpdated;
        }
        ...
        private async void View_AutoRunClicked(object sender, EventArgs e)
        {
            try
            {
                _view.SetAutoRunEnabled(false);
                _view.SetStopEnabled(true);
                _view.AddMessage("開始自動運行...");
                await _machine.StartAutoRunAsync();
                _view.AddMessage("自動運行已完成");
            }
            catch (Exception ex)
            {
